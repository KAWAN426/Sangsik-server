...
router.get("/user/bookmarks/:userId", (req, res) => PostController.getPostList(req, res, {
    bookmarks: req.params.userId,
}));
//* desc 특정 사용자의 포스트를 정렬
//* params: { postId: string, userId: string }, query: { q?: string, order?: "latest" | "popular" }
router.get("/user/:userId", (req, res) => PostController.getPostList(req, res, { authorId: req.params.userId }));
//* desc 포스트의 좋아요 수정
//* params: { postId: string, userId: string }
router.put("/toggle/like/:postId/:userId", PostController.togglePostLike);
//* desc 포스트의 북마드 수정
//* params: { postId: string, userId: string }
router.put("/toggle/bookmark/:postId/:userId", PostController.togglePostBookmark);
//* desc 새 포스트 생성
